...
export async function createDailyChallenge() {
  ...
  // Create map of existing challenges
  const existingMap = new Map(
    existingChallenges.map((challenge) => [
      challenge.challengeDate.toISOString(),
      challenge,
    ])
  )

  // Create challenges for any missing dates
  const results = [dateOnly, tomorrowDate]
    .filter((targetDate) => !existingMap.has(targetDate.toISOString()))
    .map(async (targetDate) => {
      console.log(
        `Creating challenge for ${targetDate.toISOString().split('T')[0]}`
      )
      return createChallengeForDate(targetDate)
    })

  return Promise.all(results.filter(Boolean))
}

// Helper function to create a challenge for a specific date
async function createChallengeForDate(
  challengeDate: Date
): Promise<ChallengeResult | undefined> {
  try {
    console.log('Starting createChallengeForDate function')
    ...
    if (datesWithAllData.length === 0) {
      console.error(
        'No dates with interest rate, currency, and stock data found'
      )
      return
    }
    ...
    console.log(
      `Selected date: ${selectedDate.date.toISOString().split('T')[0]}`
    )
    ...
    if (!currencyValue) {
      console.error(
        'No currency value found for the selected date and currency'
      )
      return
    }
    console.log(
    ...
    if (error.code === 'P2002') {
      ...
      if (existingChallenge) return existingChallenge as ChallengeResult
    }
    console.error('Error in createChallengeForDate:', error)
    return undefined
  }
}
